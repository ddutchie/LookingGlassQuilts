...
public class SetQuiltAngle : MonoBehaviour
{
    ...
    public void LoadQuiltInfoFromFile(string filePath)
    {
        ...
        if (furtherSplit.Length > 2)
        {
            ...
            {
                //Aspect
                string[] aspect = furtherSplit[2].Split(separatingStringsFinal, System.StringSplitOptions.RemoveEmptyEntries);
                if (aspect.Length > 2)
                {
        ...
        currentQuiltInfo.quilt_settings.viewTotal = (int)currentQuiltInfo.quilt_settings.viewX * (int)currentQuiltInfo.quilt_settings.viewY;
    ...
    public void SaveImageInfo()
    {
        ...
        string newPath = Path.Combine(Path.GetDirectoryName(currentPath),
            newfilename + "_qs" + (int)currentQuiltInfo.quilt_settings.viewX + "x" + (int)currentQuiltInfo.quilt_settings.viewY + "a" + currentQuiltInfo.quilt_settings.aspect + fileExt);
    ...
    public void SetQuiltPosition(float x)
    {
        ...
        if (currentQuiltInfo.mediaType == "photoset")
        {
            double videoTime = (double)((float)x / (float)currentQuiltInfo.quilt_settings.viewTotal) * videoPlayer.length;
            ...
            quiltMaterial?.SetVector("_QuiltVec", new Vector4(currentQuiltInfo.quilt_settings.viewX, currentQuiltInfo.quilt_settings.viewY, 0, 0));
            quiltMaterial?.SetVector("_QuiltVec2", new Vector4(currentQuiltInfo.quilt_settings.viewX, currentQuiltInfo.quilt_settings.viewY, 0, 0));
        }
        else
        {
            Vector2 quiltPos = intRotationToVector2((int)x);
            Vector2 quiltPos2 = intRotationToVector2Plus((int)x);
            quiltMaterial?.SetVector("_QuiltVec", new Vector4(currentQuiltInfo.quilt_settings.viewX, currentQuiltInfo.quilt_settings.viewY, quiltPos.x, quiltPos.y));
            quiltMaterial?.SetVector("_QuiltVec2", new Vector4(currentQuiltInfo.quilt_settings.viewX, currentQuiltInfo.quilt_settings.viewY, quiltPos2.x, quiltPos2.y));
        }
    }
    ...
    private void Update()
    {
        ...
        if (gyroEnabled)
        {
            ...
            //   if(deltaEulerAngles)
            float mappedFloat = currentQuiltInfo.viewOrderReversed ? Remap(deltaEulerAngles.y, -20.0f, 20.0f, viewTotalMinusOne, 0) : Remap(deltaEulerAngles.y, -20.0f, 20.0f, 0, viewTotalMinusOne);
        ...
        else
        {
            ...
            float mappedFloat = currentQuiltInfo.viewOrderReversed ? Remap(mousePos.x, 0, Screen.width, 0, viewTotalMinusOne) : Remap(mousePos.x, 0, Screen.width, viewTotalMinusOne, 0);
            SetQuiltPosition(mappedFloat);
