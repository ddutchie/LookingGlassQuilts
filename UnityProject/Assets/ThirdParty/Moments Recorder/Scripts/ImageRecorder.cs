...
namespace Moments
{
    ...
    public sealed class ImageRecorder : MonoBehaviour
    {
        ...
        Queue<Texture2D> m_Frames;
        RenderTexture m_RecycledRenderTexture;
        ReflectionUtils<ImageRecorder> m_ReflectionUtils;
        ...
        public void Setup(bool autoAspect, int width, int height, int fps, float bufferSize, int repeat, int quality, float scale)
        {
            ...
            m_ReflectionUtils.ConstrainRange(x => x.m_resolutionScale, scale);
        ...
        void Awake()
        {
            m_ReflectionUtils = new ReflectionUtils<ImageRecorder>(this);
        ...
        public void ConvertQuiltToGif(Texture2D quilt, SetQuiltAngle.QuiltInfo quiltInfo)
        {
            ...
            for (int i = 0; i < quiltInfo.quilt_settings.viewTotal - 1; i++)
            {
                int quiltX = (int)(i % quiltInfo.quilt_settings.viewX);
                int quiltY = (int)(i / quiltInfo.quilt_settings.viewX);
                Color[] pixels = quilt.GetPixels(0 + width * quiltX, 0 + height * quiltY, width,
                    height, 0);
                Texture2D newTexture2D = new Texture2D(width, height);
                newTexture2D.SetPixels(pixels);
                newTexture2D.Apply();
                m_Frames.Enqueue(newTexture2D);
            }
            for (int i = quiltInfo.quilt_settings.viewTotal - 1; i > 0; i--)
            {
                int quiltX = (int)(i % quiltInfo.quilt_settings.viewX);
                int quiltY = (int)(i / quiltInfo.quilt_settings.viewX);
                Color[] pixels = quilt.GetPixels(0 + width * quiltX, 0 + height * quiltY, width,
        ...
        IEnumerator PreProcess(string filename)
        {
            ...
            {
                GifFrame frame = ToGifFrame(m_Frames.Dequeue());
                frames.Add(frame);
                yield return null;
            }
        ...
        GifFrame ToGifFrame(Texture2D target)
        {
            TextureScaler.scale(target, (int)(target.width * m_resolutionScale), (int)(target.height * m_resolutionScale));
